...
- Codex app-server is owned exclusively by Alleycat (hard-fail policy):
  Codex Desktop must never spawn its own bundled `codex app-server`. The launcher's `configure_alleycat_app_server_transport` (`launcher/start.sh.template`) now exits with `notify_error` when `alleycat` is missing, when `alleycat.service` is not reachable within ~3s, or when the configured Codex websocket bridge (`ws://127.0.0.1:8390` by default; read from `~/.config/alleycat/host.toml [agents.codex]`) is not listening. The matching codex CLI wrapper at `~/dotfiles/codex/codex` refuses to start any subcommand that needs an app-server (TUI, `exec`, `resume`, `fork`, `review`, `cloud`, `apply`, `app-server daemon ...`, etc.) when alleycat is unreachable. The reason for the hard-fail: upstream `codex app-server daemon start` double-forks and reparents the daemon to `systemd --user`, so a single fallback spawn can silently leak a long-lived daemon whose `cwd` then traps every future Codex chat in whatever project directory the first spawn happened from. Escape hatches: `CODEX_DESKTOP_USE_ALLEYCAT=0` (launcher) and `CODEX_BYPASS_ALLEYCAT=1` (CLI wrapper) — debugging only; never set permanently. The complementary alleycat-side enforcement (force `UnixProxy` mode, pin `current_dir($HOME)` on every `codex` spawn) lives in `~/repos/alleycat/crates/alleycat/src/agents.rs` and is documented in `~/repos/alleycat/AGENTS.md` ("Codex app-server lifecycle"). When debugging "Codex Desktop starts every chat in the wrong directory", the first thing to check is `ps -eo pid,ppid,cmd | grep 'codex app-server'` — any process whose `PPid` is not the running `alleycat` is an orphan that needs to be killed, and the pid/socket files at `~/.codex/app-server-daemon/app-server.pid` and `~/.codex/app-server-control/app-server-control.sock` removed before restarting alleycat.
- DMG extraction:
